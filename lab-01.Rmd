...
visitors %>% count(region, sort = TRUE)
```
...
visitors %>% count(admission, setting, sort = TRUE)
```


## Question 2

_What are the variable data types? character and numeric_
...
_Which attraction had the most number of visitors in 2022? The Crown Estate, Windsor Great Park_
...
_What are the top 10 most visited attractions in 2021? Crown Estate, Royal Botanic Gardens Kew, Natural History Museum, RHS Garden Wisley, The British Museum, Tate Modern, Somerset House, Science Museums, Jeskyns Community Woodland, V&A South Kensington._
```{r Ex_c}
visitors %>% 
  arrange(desc(n_2021)) %>%
  head(n = 10)
...
_How many regions have more than 50 tourist attraction in the data set? (Hint: You will need to tabulate the data before filtering.) Scotland_
...
**NA stands for Not Available, I'm getting this for the previous answer because not every value of n_2021 is numeric, some are character**
...
**ra.rm removes all 'NA' values from calculations so that a numeric result can be outputted**
